if not http then
    printError("FoxFox requires the http API")
...
-- TODO: remove or replace some
local htmlEntities = require("htmlEntities")

local handle = fs.open("/manual.html", "r")
--local handle = http.get("https://www.lua.org/manual/5.1/manual.html")
if not handle then
    printError("Faild to create handle")
    return
end
local htmlstring = handle.readAll()
handle.close()

-- this will remove Whitespace befor and after a string
local function trimWhitespace(str)
    return str:gsub("^%s*(.-)%s*$", "%1")
end

local function removeHtmlFuckery(str)
    return htmlEntities.decode(trimWhitespace(str))
end

local limit = -1 -- Disable loop limit
local root = htmlparser.parse(htmlstring, limit)
...
local page = {}
local elements = root('*') -- get all elements


-- Define a recursive function to loop through elements and text
local function loopElementsAndText(node)
    -- Loop through child elements
    for _, element in ipairs(node.nodes) do
        -- Process the element
        table.insert(page, "e:" .. removeHtmlFuckery(element.name) .. element:textonly())

        -- Recursive call to process child elements and text
        loopElementsAndText(element)
    end

    -- Process the element's text content
    local text = node:textonly()
    if text ~= "" then
        table.insert(page, "t:" .. removeHtmlFuckery(text))
    end
end

-- Start the loop with the root element
loopElementsAndText(root)
...
local function p(n)
    ...
    -- element.name
    table.insert(page, removeHtmlFuckery(text))
...
p(root)]]
--[[
local function recursiveAdd(node)
    for _, element in ipairs(elements) do
        --table.insert(page, removeHtmlFuckery(element:gettext()))
        for k, node in ipairs(element.nodes) do
            print(k, node)
            --table.insert(page, removeHtmlFuckery(node))
        end
    end
end
...
for _, element in ipairs(elements) do
    --  element.name
    local text = removeHtmlFuckery(element:tolist())
    if text ~= "" then
        table.insert(page, element)
    end
    --[[
    local subs = element("*")
    for _, sub in ipairs(subs) do
        -- print(sub.name)
        table.insert(page, "f " .. removeHtmlFuckery(sub:textonly()))
    end

end
]]
--local width, height = term.getCursorPos()
--textutils.pagedPrint(table.concat(page, "\n"), height - 2)

-- https://github.com/msva/lua-htmlparser/tree/master
-- https://github.com/ajh123/DragonBrowser/blob/master/DargonBrowser.lua
-- https://github.com/search?q=web+browser+language%3ALua&type=repositories&l=Lua&p=3
-- https://www.lua.org/manual/5.1/manual.html
-- https://tweaked.cc/
-- https://github.com/cc-tweaked/CC-Tweaked/blob/3bf29695fb2544e74892672ce17b7d7444487a8f/projects/core/src/main/resources/data/computercraft/lua/rom/programs/help.lua#L32C1-L33

--[[
    Code that is definatlly not coppied from
    https://github.com/cc-tweaked/CC-Tweaked/blob/3bf29695fb2544e74892672ce17b7d7444487a8f/projects/core/src/main/resources/data/computercraft/lua/rom/programs/help.lua#LL145C31-L145C38
    SPDX-FileCopyrightText: 2017 Daniel Ratcliffe

    SPDX-License-Identifier: LicenseRef-CCPL
]]
local sTopic = getTitle()
